...
void searchItem(const struct Item item[], int NoOfRecs);
...
void GroceryInventorySystem(void) { //Option for user selection
    ...
    do {
        ...
        switch (option) {
            ...
            case 6:
                deleteItem(groceryitem, &records);
                pause();
                break;
                
            case 7:
                searchItem(groceryitem,records);
                pause();
                break;
        }
    } while (flag == 0);
...
int loadItem(struct Item* item, FILE* dataFile) {
    ...
    int number = fscanf(dataFile, "%d,%d,%d,%lf,%d,%[^\n]", &(*item).sku, &(*item).quantity, &(*item).minQuantity, &(*item).price, &(*item).isTaxed, (*item).name);
    
    if (number == 6) {    // returns true if six items are readed successfully
        found = 1;
    }
    return found;
}
...
void searchItem(const struct Item item[], int NoOfRecs){  //searching  item from a parth of name.
    ...
    for (i=0; i < NoOfRecs; i++){
        if (strstr(item[i].name ,name) != 0){
            displayItem(item[i],FORM);
            pause();
        }
    }
    
    if (strstr(item[i].name , name) == NULL){
